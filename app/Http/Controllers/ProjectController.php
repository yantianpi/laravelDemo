    ...
    public function projectEdit(ProjectValidateRequest $request, $id = 0) {
        ...
        switch ($action) {
            ...
            case 'doedit':
                ...
                try {
                    $oldObject = Project::findOrFail($id);
                    $dataObject->save();
                    $tmpContent = $oldObject->toJson() . ' >>> ' . $dataObject->toJson();
                    CommonController::logRecord('BASIC', $id, __FILE__, 'project update project update', $tmpContent);
            ...
            case 'doadd':
                ...
                if (!$tmpCollection->isEmpty()) {
                    ...
                    try {
                        ...
                        $projectId = DB::table('project_list')->insertGetId($tmpArray);
                        $tmpObject = Project::findOrFail($projectId);
                        $tmpContent = $tmpObject->toJson();
                        CommonController::logRecord('BASIC', $projectId, __FILE__, 'project add project add', $tmpContent);
                        return redirect("/project/edit/{$projectId}?action=edit");
                    } catch (\Exception $e) {
                        return back()->withErrors([$e->getMessage()])->withInput();
                    }
                }
